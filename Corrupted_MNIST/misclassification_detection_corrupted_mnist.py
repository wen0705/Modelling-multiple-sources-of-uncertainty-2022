...
test_in_domain_loader = torch.utils.data.DataLoader(test_indomain,batch_size=BATCH_SIZE_Test, shuffle=True)
test_OOD_loader = torch.utils.data.DataLoader(test_OOD,batch_size=BATCH_SIZE_Test, shuffle=True)
test_erased_loader = torch.utils.data.DataLoader(test_indomain_er,batch_size=BATCH_SIZE_Test, shuffle=True)
...
for batch, (data, label) in enumerate(test_in_domain_loader):
  ...
  # ACCURACY 
  acc = torch.eq(pred,label).detach().numpy()
  ...
  entropy.append(list(ent_score[label<=9]))
  differential.append(list(dff_score[label<=9]))
  datauncertainty.append(list(data_score[label<=9]))
  MI.append(list(MI_score[label<=9]))
...
with open('test0.txt', 'w') as f:
  f.write(str(entropy)+'\n')
  f.write(str(datauncertainty)+'\n')
  f.write(str(differential)+'\n')
  f.write(str(MI)+'\n')
  ...
  for idx,i in enumerate(output):
    ...
    if len(leng) >= 2:
      ...
      with open('Data.txt', 'a') as f:
        f.write(str(data[idx].data.numpy())+'\n')
...
entropy = []
MI =[]
for batch, (data, label) in enumerate(test_OOD_loader):
...
print('test accuracy OOD', accuracy_OOD/10000)
